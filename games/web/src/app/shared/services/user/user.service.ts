...
export class UserService {
  ...
  getCurrentUser$ = this.httpClient.get<Response<IUser>>('/users', { context: setNotToastSuccess }).pipe(
    tap(res => {
      this.user = res.data
      this.listen()
    }),
    switchMap((user) => {
      console.log(user.data.isPlaceCastle)
  ...
  signIn(payload: ISignInPayload) {
    return this.httpClient.post<Response<ISignIn>>('/users/signin', payload)
      .pipe(
        tap(async ({ data }) => {
