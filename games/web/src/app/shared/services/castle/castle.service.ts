...
export class CastleService {
  ...
  getCastle$ = this.httpClient.get<Response<ICastle[]>>('/castles', { context: setNotToastSuccess })
  ...
  constructor() {
    combineLatest([this.buildingService.buildings$, this.unitService.units$])
      .subscribe(([_, units]) => {
        const population = this.buildingService.getBuildingByKey(DEFAULT_BUILDING.KEY.DWELLINGS)?.upgrade.current.generate || 0
  ...
  getDetail(id: string) {
    return this.httpClient.get<Response<ICastleDetail>>(`/castles/detail/${id}`, { context: setNotToastSuccess }).pipe(
      tap(({ data }) => {
